...
fpl2122%>%
  ...
  filter(name  %in% c("Ben Davies"))
fpl2122%>%
  group_by(name, team)%>%
  filter(name  %in% c("Ben Davies"))
#Weird its not showing the liverpool rows, but if i go to the dataset and search I can see there are also rows with Liverpool
#So we will jsut remove these
...
#This would identify the top most influential player per gameweek for each team
#top_influence <- 
#  fpldata%>%
#  group_by(Season, GW, team)%>%
#  summarize(Influence = influence, Points = total_points, Name = name)%>%
#  slice(which.max(Influence))

#Lets try making a tab for each team and then showing a table of Influence and Total points per player
